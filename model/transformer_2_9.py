...
class KernelEstimate(nn.Module):
    ...
    def forward(self, blur, sharp):
        ...
        kern[kern < 0] = 0
        b, n, ph, pw = kern.size()
        kern = kern.view(b*n, -1)
        div = kern.sum(1, keepdim=True)
        div[div == 0] = self.eps
        kern /= div
        kern = kern.view(b, n, ph, pw)
...
class Xformer2(nn.Module):
    def __init__(self, pat_size=16, in_channels=3, in_sharp_blur_feats=16, out_sharp_blur_feats=2, blur_sim_feats=8, num_heads=9, *args, **kwargs) -> None:
        super().__init__(*args, **kwargs)
